...
def create_app():
    ...
    with app.app_context():
        ...
        if not owner:
            owner = User(
                name=os.environ.get("SYSTEM_OWNER_NAME"),
                username=os.environ.get("SYSTEM_OWNER_USERNAME"), 
                role=os.environ.get("SYSTEM_OWNER_ROLE"),
                email=os.environ.get("SYSTEM_OWNER_EMAIL"),
                status="Active"
            )
            owner.set_password(os.environ.get("SYSTEM_OWNER_PASSWORD"))
